...
const load=()=>{
  if(!fs.existsSync(F))return{users:[],tasks:[],notifications:[],pending:[],chats:[],submissions:[],task_groups:[],task_comments:[],reset_tokens:[],typing:[],_seq:{users:0,tasks:0,notifications:0,pending:0,chats:0,submissions:0,task_groups:0,task_comments:0}};
...
function init(){
  ...
  if(d.users.length===0){
    ...
    d.tasks=[];d.notifications=[];d.chats=[];d.pending=[];d.submissions=[];d.task_groups=[];d.task_comments=[];d.reset_tokens=[];d.typing=[];
